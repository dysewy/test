...
import kk.jdbcfile;
import kk.学员信息删除;
public class 学员信息删除 extends JFrame implements ActionListener{
	JPanel p=new JPanel();
	...
	static String[] col_all= {"姓名","性别","身份证号码","车型","电话"};
	...
	static String cx(String s) {
		...
		try {
			...
			while(rs.next()) {
				...
				String rowvalues[]= {name,gender,id,type,phone};
				tm.addRow(rowvalues);
			}
		} catch (Exception e) {
...
//		tp.setBorder(BorderFactory.createLineBorder(Color.gray,3));
	...
	public void actionPerformed(ActionEvent e) {
		...
		String sql1="select * from stu where gender='"+strGender+"'";
		String sql2="select * from stu where name like '%"+strName+"%'";
		String sql3="select * from stu where type like '"+strType+"%'";
		...
		if(e.getSource()==B1) {
			if(strName.length()==0&strType.length()==0) {
				...
				clear();}}else if(e.getSource()==B2) {
					...
					if(strT1.length()>0) {
						jdbcfile jdbc;
						String sql4="select name from stu where ID='"+strT1+"'";
						String sql7="delete from stu where ID='"+strT1+"'";
						try {
							jdbc = new jdbcfile();
