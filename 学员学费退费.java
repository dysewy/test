...
import kk.jdbcfile;
import kk.学员学费退费;
public class 学员学费退费 extends JFrame implements ActionListener{
	JPanel p=new JPanel();
	...
	static String phone;
	static String money;
	static String sql="select * from stu_money";
	static String cx(String s) {
		...
		try {
			...
			while(rs.next()) {
				...
				String rowvalues[]= {name,gender,id,number,phone,money};
				tm.addRow(rowvalues);
			}
		} catch (Exception e) {
...
//		tp.setBorder(BorderFactory.createLineBorder(Color.gray,3));
	...
	public void actionPerformed(ActionEvent e) {
		...
		String sql1="select * from stu_money where gender='"+strGender+"'";
		String sql2="select * from stu_money where name like '%"+strName+"%'";
		String sql3="select * from stu_money where number like '"+number+"%'";
		...
		if(e.getSource()==B1) {
			if(strName.length()==0&strType.length()==0) {
				...
				clear();}}else if(e.getSource()==B2) {
					...
					if(strT1.length()>0) {
						jdbcfile jdbc;
						String sql4="select name from stu_money where ID='"+strT1+"'";
						String sql7="delete from stu_money where ID='"+strT1+"'";
						try {
							jdbc = new jdbcfile();
							...
							if(rs.next()) {
								...
								if(c==1) {
									学员学费退费 iu=new 学员学费退费();
									((AbstractButton) iu.Tname).setText(strgetName);
								}else {
